...
def evaluate(tree, genv, lenv)
  ...
  when "lit"
    tree[1]
  when "+"
    evaluate(tree[1], genv, lenv) + evaluate(tree[1], genv, lenv)
  when "-"
    evaluate(tree[1], genv, lenv) - evaluate(tree[1], genv, lenv)
  when "*"
    evaluate(tree[1], genv, lenv) * evaluate(tree[1], genv, lenv)
  ...
  when "stmts"
    ...
    last = nil
    while tree[i]
      list = evaluate(tree[i], genv, lenv)
      i += 1
    end
    last
  when "var_assign"
  ...
  when "ary_new"
    ...
    ary
  when "ary_ref"
    ary = evaluate(tree[i + 1], genv, lenv)
    idx = evaluate(tree[i + 2], genv, lenv)
    ary[idx]
  when "ary_assign"
    ary = evaluate(tree[i + 1], genv, lenv)
    idx = evaluate(tree[i + 2], genv, lenv)
    val = evaluate(tree[i + 3], genv, lenv)
    ary[idx] = val
  when "hash_new"
    hsh = {}
    i = 0
